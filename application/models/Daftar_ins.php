	...
	function get_ins_ku(){
		...
		foreach ($query->result_array() as $daftarakun) {
			...
			$result.="<td><div class='btn-group-sm'>
				...
				<button class='btn btn-success' title='Approve'>A</button>
				<button class='btn btn-warning' title='Perbaiki'>P</button>
				<button class='btn btn-danger' title='Reject'>R</button></div></td>";
			$result.="</tr>";
			$no++;
		}
		return $result;
